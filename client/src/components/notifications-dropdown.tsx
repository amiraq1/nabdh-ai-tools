  ...
  const generateNotifications = (): Notification[] => {
    ...
    suppliers.forEach((supplier) => {
      ...
      if (balance > HIGH_BALANCE_THRESHOLD) {
        notifications.push({
          ...
          type: "high_balance",
          title: "رصيد مرتفع",
          description: `المورد "${supplier.name}" لديه رصيد مرتفع (${balance.toLocaleString('ar-IQ')} د.ع)`,
      ...
      if (balance < NEGATIVE_BALANCE_THRESHOLD) {
        notifications.push({
          ...
          description: `المورد "${supplier.name}" له رصيد مستحق (${Math.abs(balance).toLocaleString('ar-IQ')} د.ع)`,
          supplierId: supplier.id,
          supplierName: supplier.name,
          severity: "critical",
          createdAt: new Date(),
    ...
    recentLargeTransactions.forEach((transaction) => {
      ...
      notifications.push({
        ...
        title: "معاملة كبيرة",
        description: `تم تسجيل ${transaction.type === 'debit' ? 'مشتريات' : 'دفعة'} بمبلغ ${transaction.amount.toLocaleString('ar-IQ')} د.ع`,
